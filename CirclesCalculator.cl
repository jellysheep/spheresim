...
__kernel void randomFill(__global struct Circle* circle, __global uint* m_z, __global uint* m_w,
	...
	circle[gid].size = s2.s0+((s2.s1-s2.s0)*uGetUniform(m_z, m_w));
...
__kernel void moveStep(__global struct Circle* circle, __global int* num,
						...
						__global floating_type* delta_t_)
{
	floating_type_vector s = *size, force, acceleration;
	floating_type reduced = *elastic, gravity = *g, delta_t = *delta_t_;
	...
	for(int i = (*num)-1; i>id; i--){
		...
		if (d < both_r) {
			...
			force = 4.0/3.0*E_*pow(R, 1.0/2.0)*pow(d_, 3.0/2.0) *normalize(d_pos);
