...
void message(const QString& title, const QString& str, QMessageBox::Icon icon=QMessageBox::Information){
	QMessageBox mb;
...
void infoMessage(const QString& str){
	message("Info", str, QMessageBox::Information);
}

void errorMessage(const QString& str){
	message("Error", str, QMessageBox::Critical);
}
...
FileCalculator::FileCalculator():Calculator(){
	...
	if(str == ""){
		errorMessage(tr("File could not be opened!"));
		errorExit();
		return;
	}
	...
	if (f.is_open())
	{
		...
		saveInVar(f, _3d);
			...
			if((_3d!=0) != use3D){
				if(_3d == 0){
					errorMessage(tr("You have to open this file with 2D mode."));
				}else{
					errorMessage(tr("You have to open this file with 3D mode."));
				}
				errorExit();
				return;
			}
